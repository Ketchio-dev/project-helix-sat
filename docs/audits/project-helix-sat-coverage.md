...
- Formats: grid_in=10, single_select=65, student_produced_response=4
...
- PASS Minimum math student-response slice: 14 student-produced-response items (threshold: 14+ student-produced-response items)
- PASS Multi-format runtime floor: 3 shipped formats with math SPR=true (threshold: 2+ shipped formats with math student-produced response enabled)
- PASS Default module floor: 14 item default module (threshold: 14+ item default module)
- PASS Section retake-resistance floor: reading_writing=33, math=46 (threshold: Each section carries at least 28 items (2x the default module size))
...
- Module simulation: 14 items, examMode=true, timeLimitSec=1400, sections=math=14
- Session review gated until completion: true

## Major risks
- Module simulation is 14 math items — real SAT modules are 22–27 items, so the current shape is still much shorter than exam-realistic.
