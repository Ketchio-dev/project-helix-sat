...
1. **Format realism is still bounded.**
   - 5 Math items use `grid_in`, but the rest of the bank still uses `single_select`.
...
2. **Module simulation is still unlike a real digital SAT module.**
   - It is section-separated and 8 items long, but it still remains much shorter than a real module-length section.
