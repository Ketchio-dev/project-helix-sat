...
- three new math student-produced-response items across linear functions, ratios/rates, and statistics/probability
...
### Format realism
- 14 Math items now use student-produced-response formats across `grid_in` and `student_produced_response`
...
- Module simulation now defaults to a **14-item** section-specific exam block
- The learner shell now also exposes **optional 20-item extended modules** for denser Reading/Writing and Math practice plus an **exam profile** at 27 Reading/Writing items / 22 Math items, while the standard audit snapshot stays on the default 14-item shape
...
- `npm run audit:helix:bars` enforces the current release bars (full blueprint coverage, 14 math student-produced responses, a multi-format runtime floor, zero singleton lanes, a 14-item default module floor, and 2x section retake-resistance)
...
1. **Module simulation is still shorter than the real digital SAT.**
   - The default shipped module is now 14 items long, and the optional extended section profiles are 20 items, so all paths remain materially shorter than a real SAT module.
2. **Format realism is broader but still bounded.**
- 14 Math items now use student-produced-response formats across `grid_in` and `student_produced_response`, and the shipped Math modules now force a repeated numeric-entry slice (3 student responses in the 14-question standard block, 5 in the 20-question extended block, 6 in the 22-question exam profile), but the overall bank is still mostly `single_select`.
...
2. **Module simulation still compresses structure too aggressively.**
   - Fourteen default section-specific items are easier to complete, and the new optional 20-item section profiles help, but none of the current paths yet feel like a true SAT module.
...
1. Raise section-specific module length beyond the current 14-item default / 20-item extended profiles toward a more exam-realistic shape.
...
Today the product is a believable **two-section SAT prototype** with a 79-item bank, full ontology-slice coverage in the audit, a 14-item math numeric-entry slice that now appears repeatedly inside shipped Math modules, authored remediation blueprints across the current curriculum map, an all-skill middle-pack lesson baseline plus an 11-skill full-pack cohort, a 13-item onboarding baseline diagnostic, a 14-item default module path, optional 20-item extended modules for both sections, and a larger exam profile at 27 Reading/Writing items / 22 Math items. It is much closer to a real curriculum-backed SAT product than it was a few slices ago, but module realism and deeper lesson depth are still the clearest next bottlenecks.
