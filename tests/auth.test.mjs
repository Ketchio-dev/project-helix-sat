...
describe('auth', () => {
  it('demo user can login with demo123 and get a valid token', () => {
    const store = createStore();
    const result = store.loginUser({ email: 'mina@example.com', password: 'demo123' });
  ...
  it('loginUser throws for unknown email', () => {
    const store = createStore();
    assert.throws(
      () => store.loginUser({ email: 'nobody@example.com', password: 'demo123' }),
      /Invalid credentials/i,
    );
  });
  ...
  it('registerUser creates a new user and returns token', () => {
    const store = createStore();
    const result = store.registerUser({ name: 'Test User', email: 'test@example.com', password: 'pass123' });
    assert.ok(result.user);
    assert.ok(result.token);
  ...
  it('registered user can login with their password', () => {
    const store = createStore();
    store.registerUser({ name: 'Test User', email: 'test@example.com', password: 'pass123' });
    const result = store.loginUser({ email: 'test@example.com', password: 'pass123' });
    assert.ok(result.user);
    assert.ok(result.token);
    assert.equal(result.user.email, 'test@example.com');
  });
  ...
  it('registerUser rejects duplicate email', () => {
    const store = createStore();
    store.registerUser({ name: 'First', email: 'dup@example.com', password: 'pass123' });
    assert.throws(
      () => store.registerUser({ name: 'Second', email: 'dup@example.com', password: 'pass456' }),
      /already registered/i,
    );
  });

  it('registerUser rejects invalid role', () => {
    const store = createStore();
    assert.throws(
      () => store.registerUser({ name: 'Bad Role', email: 'badrole@example.com', password: 'pass123', role: 'superadmin' }),
      /Invalid role/i,
    );
  });
  ...
  it('registered student gets initialized profile, skillStates, errorDna', () => {
    const store = createStore();
    const result = store.registerUser({ name: 'Student', email: 'student@example.com', password: 'pass123' });
